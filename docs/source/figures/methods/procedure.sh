...
i=$(($i+1))
echo ".. figure::"
for file in $(ls /data2/rivasas2/singleNucleosome/secondBatch/counts/expected_counts/*svg | grep $histone); do
	ln -s $file .
	name=$(echo $file | awk '{n=split($0,a,"/");print a[n]}')
	echo ".. image:: https://132.239.135.28/public/nucChIP/files/methods/${name}"
	echo "   :width: 45%"
done
