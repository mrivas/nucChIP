...
i=1
echo ".. figure::"
ln -f -s /data2/rivasas2/singleNucleosome/alternativeSplicing/SE/summary_day0/summary/internal_exons/exon_fpkm.svg .
name=exon_fpkm.svg
echo ".. image:: https://132.239.135.28/public/nucChIP/files/coverage/exons/${name}"
echo "   :width: 45%"
...
i=$(($i+1))
echo ".. figure::"
for file in $(ls /data2/rivasas2/singleNucleosome/secondBatch/counts/counts_per_nucleosome/distance_from_exon_overExpected/AS/*1.nuc*svg | grep $histone); do
	ln -f -s $file .
	name=$(echo $file | awk '{n=split($0,a,"/");print a[n]}')
	echo ".. image:: https://132.239.135.28/public/nucChIP/files/coverage/exons/${name}"
	echo "   :width: 45%"
