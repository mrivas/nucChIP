...
for file in $( ls /data2/rivasas2/singleNucleosome/secondBatch/v-plots/AS_exons/*svg | grep $hist) ; do
	...
	if [ -f ${hist}_fragmentDist_exons.rst ]; then
		nLine=$(wc -l ${hist}_fragmentDist_exons.rst | awk '{print $1}')
		mod=$(($nLine%15))
	...
	if [ "$mod" == 0 ]; then 
		echo ".. figure::" >> ${hist}_fragmentDist_exons.rst
	fi
	...
	if [ "$mod" == 11 ]; then
		echo "Figure ${nFig}: Fragment distribution of $name on internal exons." >> ${hist}_fragmentDist_exons.rst
		echo "" >> ${hist}_fragmentDist_exons.rst
	fi
...
echo "Figure 2: Ratios of fragment distribution of $name on internal exons." >> ${hist}_fragmentDist_exons.rst
echo "" >> ${hist}_fragmentDist_exons.rst
